...
#     try:
#         # Process the PDF file with Textract
...
#             # For PDFs, use start_document_analysis instead of analyze_document
...
#             print(f"Started document analysis for {s3_file_key}. Job ID: {job_id}")
#             # return [{"s3_file_key": s3_file_key, "job_id": job_id, "message": "Processing started. Use GetDocumentAnalysis to fetch results."}]
...
#         print(f"Text extraction completed successfully. Results saved to {output_path}")
#         return [{"s3_file_key": s3_file_key, "output_path": output_path, "extracted_text": extracted_text}]
...
def process_images_with_textract(output_folder, s3_file_key):
    ...
    try:
        if file_extension == ".pdf":
            ...
            response = textract_client.start_document_analysis(
                DocumentLocation={'S3Object': {'Bucket': bucket_name, 'Name': s3_file_key}},
                FeatureTypes=["TABLES", "FORMS"]
            )
            job_id = response["JobId"]
