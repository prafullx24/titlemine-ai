...
def get_files_by_project(project_id):
    ...
    cur = conn.cursor()
    query = "SELECT id, user_id, file_name, s3_url, ocr_status FROM public.files WHERE project_id = %s AND ocr_status != 'completed'"
...
def download_file(file_info):
    ...
    file_id, user_id, file_name, s3_url, _ = file_info
    file_extension = os.path.splitext(file_name)[1]
    file_name = f"download_pdf_{user_id}_{file_id}{file_extension}"
...
# def save_and_update_ocr_data(file_id, extracted_data):
#     """Save extracted OCR data to the database and update status."""
#     conn = psycopg2.connect(**DB_CONFIG)
#     cur = conn.cursor()
...
#         """
#         cur.execute(query, (file_id, json.dumps(extracted_data), extracted_text))
...
def batch_ocr(project_id):
    ...
    valid_files = [(file_id, file_path) for file_id, file_path in downloaded_files if file_path]
    
    with ThreadPoolExecutor() as executor:
        extracted_data_list = list(executor.map(lambda x: (x[0], extract_text_with_confidence(x[1])), valid_files))
    
    with ThreadPoolExecutor() as executor:
        executor.map(lambda x: save_and_update_ocr_data(x[0], x[1]), extracted_data_list)
